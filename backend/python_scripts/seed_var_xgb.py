...
#lag features for all maturities, 5 lags for each maturity
features = []
for mat in matList:
    for lag in range(1, 6):
        features.append(f"{mat}_{lag}")
...
def createRow(df, index):
    lagAll = []

    for targetMat in matList:
        lag1 = df[targetMat].iloc[index]
        lag2 = df[targetMat].iloc[index-1]
        lag3 = df[targetMat].iloc[index-2]
        lag4 = df[targetMat].iloc[index-3]
        lag5 = df[targetMat].iloc[index-4]
...
def createPredictions(df):
    dates = []
    asOfDate = df["Date"].iloc[-1]
    forecastDates = pd.bdate_range(asOfDate + pd.offsets.BDay(1), periods=20)
    current = pd.DataFrame([createRow(df, len(df.index)-1)], columns=features)

    #create one model for each maturity and horizon
    for mat in matList:
        for horizon in horizons:
            table = createTable(df, mat, horizon)
            ...
            predicted = model.predict(current)[0]
