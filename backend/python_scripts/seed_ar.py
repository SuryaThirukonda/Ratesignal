...
def createPredictions(df):
    ...
    #create an AR model with 5 lags for each maturity and window
    for start in windowStarts:
        asOfDate = df.index[start]

        for mat in matList:
            train = df[mat].iloc[:start + 1].reset_index(drop=True)
            model = AutoReg(
                endog=train,
                lags=5,
                trend="c"
            )
            forecast = model.fit().forecast(steps=20).to_numpy()
